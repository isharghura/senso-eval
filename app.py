...
for model, res in model_results.items():
    ...
    total = len(res)
    failed_count = sum(1 for r in res if r.get("issues"))
    quality_scores = [r["quality_score"] for r in res if r["quality_score"] is not None]
    consistency_scores = [r["consistency_score"] for r in res if r["consistency_score"] is not None]
...
for model, res in model_results.items():
    failures = [r for r in res if r.get("issues")]
    if failures:
        st.subheader(f"{model.upper()}")
        for f in failures:
...
st.divider()
st.caption("SensoEval - Model comparison")
